...
- From Astro Reports, the exporter accepts the current galaxy, a comma-separated galaxy list, `ALL`, or `RESUME`. It exhaustively partitions capped results by terrain, solar position, and planet/moon; uploads each completed galaxy immediately; and checkpoints progress locally so a weekly all-galaxy run can resume after interruption. Scanning `ALL` imports `B00` through `B49`, so every galaxy containing results becomes available in the Mini App switcher.
- From Astro Empires' Guild Reports page, the secure exporter can scan every available guild across Bases, Fleets, and Moving Fleets. It confirms before starting, paces and retries the report requests, offers cancellation, partitions results by their coordinate galaxy, and uploads each galaxy separately under the same approved APP access scope.
