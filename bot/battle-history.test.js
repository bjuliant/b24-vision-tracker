...
test("active occupations can span imported maps without crossing chat scope", async () => {
  const calls = [];
  const rows = [
    row({ map_id: "b23", coord: "B23:14:64:30", state: "occupied", owner_player: "One", observed_at: "2026-07-15T09:00:00Z" }),
    row({ map_id: "b24", coord: "B24:14:64:30", state: "occupied", owner_player: "Two", observed_at: "2026-07-15T10:00:00Z" }),
    row({ map_id: "b39", coord: "B39:14:64:30", chat_id: "chat-b", state: "occupied", owner_player: "Leak", observed_at: "2026-07-15T11:00:00Z" })
  ];
  const reader = createBattleHistoryReader(async (table, filters, options) => {
    calls.push({ table, filters, options });
    return table === "b24_occupations" ? rows : [];
  });

  const occupations = await reader.activeOccupations({ authorized: true, mapId: "", chatId: "chat-a" });

  assert.deepEqual(occupations.map((entry) => entry.coord), ["B24:14:64:30", "B23:14:64:30"]);
  calls.forEach((call) => {
    assert.equal(call.options.includeMap, false);
    assert.equal(call.filters.chat_id, "eq.chat-a");
  });
});
